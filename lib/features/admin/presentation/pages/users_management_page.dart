...
class _GestionUtilisateursPageState extends State<GestionUtilisateursPage> {
          ...
          children: [
            SizedBox(
              width:
                  ...
                  56, // Total width minus the filter button and spacing
              child: Container(
                height: 46,
  ...
  Widget _buildStatsRow() {
    return BlocBuilder<UserManagementCubit, UserManagementState>(
      builder: (context, state) {
        ...
        final active = state.users
            .where((u) => u.status.toLowerCase().contains('actif'))
            .length;
        final pending = state.users
            .where((u) => u.status.toLowerCase().contains('attente'))
            .length;
        final candidats = state.users
            .where((u) => u.role.toLowerCase() == 'candidat')
            .length;
  ...
  void _openUserForm(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) =>
          BlocProvider.value(value: cubit, child: const _UserFormSheet()),
    );
  }
}
